...
function draw() {
 ...
 // BACK (z = -hd)
  ...
  // FLOOR (y = +hh)
room.beginShape(room.QUADS);
room.vertex(-hw,  hh,  hd, UV.floor.u0, UV.floor.v0);
room.vertex( hw,  hh,  hd, UV.floor.u1, UV.floor.v0);
...
room.endShape();
  ...
  room.vertex(hw, -hh, hd, UV.ceil.u1, UV.ceil.v1);
  room.vertex(-hw, -hh, hd, UV.ceil.u0, UV.ceil.v1);
  room.endShape();

  // LEFT (x = -hw)
  room.beginShape(room.QUADS);
  room.vertex(-hw, -hh, hd, UV.left.u1, UV.left.v0);
  room.vertex(-hw, -hh, -hd, UV.left.u0, UV.left.v0);
  room.vertex(-hw, hh, -hd, UV.left.u0, UV.left.v1);
  room.vertex(-hw, hh, hd, UV.left.u1, UV.left.v1);
  room.endShape();

  // RIGHT (x = +hw)
  room.beginShape(room.QUADS);
  room.vertex(hw, -hh, -hd, UV.right.u1, UV.right.v0);
  room.vertex(hw, -hh, hd, UV.right.u0, UV.right.v0);
  room.vertex(hw, hh, hd, UV.right.u0, UV.right.v1);
